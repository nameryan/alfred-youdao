...
### 注册有道云API步骤
> 目前有道API已整体迁移到有道云，因此需要前往[有道云](https://ai.youdao.com)注册

1. 左侧菜单自然语言翻译->翻译实例，创建实施（实例类型选择文本翻译）
2. 左侧菜单应用管理->我的应用，创建应用（接入方式选择API）
3. 在弹出的“应用实例添加”对话框中选择刚才创建的翻译实例
4. 创建应用成功，复制应用ID和应用密钥到alfred的设置对话框（上图所示）
